/*
    ...
    And was a little bit modified by me to allocate the thread in the stack instead of the heap and to be compatible with this patch.
*/
...
// Modified
static u8 iruserSharedMemBuffer[0x1000] __attribute__((aligned(0x1000)));//allows to align a static memory block
//static Thread iruserThread;
static Handle iruserThreadHandle;
static u8 iruserThreadStack[0x400] __attribute__((aligned(8)));//allows to align a static memory block
// End
...
Result cppInit(void)
{
    ...
    iruserSharedMemAddr = (SharedMemory*)iruserSharedMemBuffer;
    ...
    // }
    // End
    ret = svcCreateThread(&iruserThreadHandle, iruserThreadFunc, 0,(u32*)(iruserThreadStack + sizeof(iruserThreadStack)), 0x28, 0);
...
void cppExit()
{
    ...
    svcWaitSynchronization(iruserThreadHandle, 20 * MILLIS);
    svcCloseHandle(iruserThreadHandle);
    // End
}
...
static void iruserThreadFunc(void* param) {
    ...
    while (true) {
        ...
        while (iruserSharedMemAddr->header.connection_status == 2) {
            ...
            // sleep to avoid slowdowns in Ocarina of Time 3D, at the cost of a slight input lag from the CPP
            svcSleepThread(16 * MILLIS);
        }
        disconnect:
        svcCloseHandle(connstatus_event);
        svcCloseHandle(recv_event);
